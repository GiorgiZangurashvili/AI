    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghosts_most_likely_positions = []
        for distribution in livingGhostPositionDistributions:
            most_likely_position = max(distribution, key=distribution.get)
            ghosts_most_likely_positions.append(most_likely_position)

        min_distance = float('inf')
        closest_ghost_position = None
        for ghost_position in ghosts_most_likely_positions:
            possible_distance = self.distancer.getDistance(pacmanPosition, ghost_position)
            if possible_distance < min_distance:
                min_distance = possible_distance
                closest_ghost_position = ghost_position

        return min(legal, key=lambda action: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action),
